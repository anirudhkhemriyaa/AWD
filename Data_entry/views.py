...
def home(request):
    ...
    if request.method == "POST":
        ...
        model_name = (request.POST.get("model_name") or "").lower()
        ...
        try:
            ...
            History.objects.create(
                ...
                work="Import",
                data=file_path.name if file_path else model_name,
                status="failed",
                error_logs=str(e)
            )
        ...
        try:
            enforce(user, "import", sub.plan.import_limit_per_day, count=1)
        ...
        messages.success(request, "Your data import is in processing, you will be notified.")
        return redirect("home")
...
def export(request):
    if request.method == "POST":
        ...
        try:
            enforce(user, "export", sub.plan.export_limit_per_day, count=1)
        except PermissionDenied as e:
            messages.error(request, str(e))
            return redirect("export")
        ...
        messages.success(request, "Your data export is in processing, you will be notified.")
        return redirect("export")
